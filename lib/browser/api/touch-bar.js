...
TouchBar.TouchBarButton = class TouchBarButton extends TouchBarItem {
  constructor (config = {}) {
    ...
    const { label, backgroundColor, icon, iconPosition = 'overlay', click } = config
...
TouchBar.TouchBarColorPicker = class TouchBarColorPicker extends TouchBarItem {
  constructor (config = {}) {
    ...
    const { availableColors, selectedColor, change } = config
...
TouchBar.TouchBarGroup = class TouchBarGroup extends TouchBarItem {
  constructor (config = {}) {
    ...
    const defaultChild = (items instanceof TouchBar) ? items : new TouchBar(items)
...
TouchBar.TouchBarLabel = class TouchBarLabel extends TouchBarItem {
  constructor (config = {}) {
    super()
    const { label, textColor } = config
    this._addImmutableProperty('type', 'label')
    this._addLiveProperty('label', label)
    this._addLiveProperty('textColor', textColor)
  }
}
...
TouchBar.TouchBarPopover = class TouchBarPopover extends TouchBarItem {
  constructor (config = {}) {
    super()
    const { label, icon, showCloseButton = true, items } = config
    this._addImmutableProperty('type', 'popover')
    this._addLiveProperty('label', label)
    this._addLiveProperty('icon', icon)
    this._addLiveProperty('showCloseButton', showCloseButton)
    const defaultChild = (items instanceof TouchBar) ? items : new TouchBar(items)
    this._addLiveProperty('child', defaultChild)
    this.child.ordereredItems.forEach((item) => item._addParent(this))
  }
...
TouchBar.TouchBarSlider = class TouchBarSlider extends TouchBarItem {
  constructor (config = {}) {
    super()
    const { label, minValue = 0, maxValue = 100, value = 50, change } = config
...
TouchBar.TouchBarSpacer = class TouchBarSpacer extends TouchBarItem {
  constructor (config = {}) {
    ...
    this._addImmutableProperty('size', size)
  }
}
...
TouchBar.TouchBarSegmentedControl = class TouchBarSegmentedControl extends TouchBarItem {
  constructor (config = {}) {
    super()
    const { segmentStyle = 'automatic', segments, selectedIndex = 0, mode = 'single', change } = config
...
TouchBar.TouchBarScrubber = class TouchBarScrubber extends TouchBarItem {
  constructor (config = {}) {
    super()
    const { items, selectedStyle = 'none', overlayStyle = 'none', showArrowButtons = false, continuous = true, mode = 'free' } = config
    let { select, highlight } = config
    this._addImmutableProperty('type', 'scrubber')
    this._addLiveProperty('items', items)
    this._addLiveProperty('selectedStyle', selectedStyle)
    this._addLiveProperty('overlayStyle', overlayStyle)
    this._addLiveProperty('showArrowButtons', showArrowButtons)
    this._addLiveProperty('mode', mode)
    this._addLiveProperty('continuous', continuous)
