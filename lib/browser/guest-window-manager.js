...
const canAccessWindow = function (sender, target) {
  return isChildWindow(sender, target) || isScriptableWindow(sender, target)
}
...
handleMessage('ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_POSTMESSAGE', (event, guestContents, message, targetOrigin) => {
  ...
  if (targetOrigin === '*' || isSameOrigin(guestContents.getURL(), targetOrigin)) {
    ...
    const sourceOrigin = new URL(event.sender.getURL()).origin
