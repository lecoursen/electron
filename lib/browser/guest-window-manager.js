...
const getGuestWebContents = function (guestId) {
...
ipcMainUtils.handle('ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_METHOD', (event, guestId, method, ...args) => {
  const guestContents = getGuestWebContents(guestId)
...
ipcMainInternal.handle('ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_POSTMESSAGE', (event, guestId, message, targetOrigin, sourceOrigin) => {
  const guestContents = getGuestWebContents(guestId)
...
ipcMainInternal.handle('ELECTRON_GUEST_WINDOW_MANAGER_WEB_CONTENTS_METHOD', (event, guestId, method, ...args) => {
  const guestContents = getGuestWebContents(guestId)
  assertCanAccessWindow(event.sender, guestContents)
...
ipcMainUtils.handleSync('ELECTRON_GUEST_WINDOW_MANAGER_WEB_CONTENTS_METHOD', (event, guestId, method, ...args) => {
  const guestContents = getGuestWebContents(guestId)
  assertCanAccessWindow(event.sender, guestContents)
